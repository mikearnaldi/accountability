...
export const Route = createFileRoute("/companies/$companyId/journal-entries/$entryId/edit")({
  component: EditJournalEntryPageWithAuth,
  beforeLoad: async ({ params }) => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (!token) {
        throw redirect({
          to: "/login",
          search: { redirect: `/companies/${params.companyId}/journal-entries/${params.entryId}/edit` },
          replace: true
        })
      }
    }
 ...
 */
function EditJournalEntryPageWithAuth(): React.ReactElement {
  const { companyId, entryId } = Route.useParams()
  return (
    <AuthGuard redirectTo={`/companies/${companyId}/journal-entries/${entryId}/edit`}>
