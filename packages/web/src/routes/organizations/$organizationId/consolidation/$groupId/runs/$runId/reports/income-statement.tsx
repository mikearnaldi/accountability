/**
 * Consolidated Income Statement Report Route
 *
 * Displays the consolidated income statement from a completed consolidation run,
 * showing Revenue, Expenses, and Net Income with NCI attribution per ASC 220.
 *
 * Route: /organizations/:organizationId/consolidation/:groupId/runs/:runId/reports/income-statement
 */
...
interface ConsolidatedIncomeStatementReport {
  ...
  readonly revenue: ConsolidatedReportSection
  readonly costOfSales: ConsolidatedReportSection
  readonly grossProfit: number
  readonly operatingExpenses: ConsolidatedReportSection
  readonly operatingIncome: number
  readonly otherIncomeExpense: ConsolidatedReportSection
  readonly incomeBeforeTax: number
  readonly taxExpense: number
  readonly netIncome: number
  readonly netIncomeAttributableToParent: number
  readonly netIncomeAttributableToNCI: number
}
...
export const Route = createFileRoute(
  ...
  beforeLoad: async ({ context, params }) => {
    if (!context.user) {
      throw redirect({
        to: "/login",
        search: {
          redirect: `/organizations/${params.organizationId}/consolidation/${params.groupId}/runs/${params.runId}/reports/income-statement`
        }
      })
    }
  },
  ...
  component: ConsolidatedIncomeStatementPage
})
...
function ConsolidatedIncomeStatementPage() {
  ...
  const [report, setReport] = useState<ConsolidatedIncomeStatementReport | null>(null)
  ...
  const breadcrumbItems = [
    ...
    },
    {
      ...
      href: `/organizations/${params.organizationId}/consolidation/${params.groupId}/runs/${params.runId}/reports/income-statement`
    }
  ]
  ...
  const handleLoadReport = async () => {
    ...
    try {
      ...
      if (result.error === "not_implemented") {
        setError("Consolidated income statement generation is not yet implemented. The report will be available in a future release.")
      } else if (result.error || !result.report) {
        setError("Failed to load the income statement report. Please try again.")
      } else {
  ...
  return (
    ...
    >
      <div data-testid="consolidated-income-statement-page">
        {/* Page Header */}
        <div className="mb-6">
          <Link
        ...
        {isRunCompleted && !hasLoaded && (
          <div className="mb-6">
            ...
            >
              {isLoading ? "Loading Report..." : "Load Income Statement"}
            </Button>
          </div>
        )}
        ...
        {report && (
          <IncomeStatementReportDisplay report={report} />
        )}
      </div>
    </AppLayout>
  )
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">
                <Tooltip content="Consolidated amount in reporting currency">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            ...
            <SectionHeader title="OTHER INCOME (EXPENSE)" />
            ...
            {/* Attribution */}
            <SectionHeader title="NET INCOME ATTRIBUTABLE TO:" />
            <tr className="hover:bg-gray-50">
              <td className="whitespace-nowrap py-2 text-sm text-gray-900" style={{ paddingLeft: "40px", paddingRight: "24px" }}>
